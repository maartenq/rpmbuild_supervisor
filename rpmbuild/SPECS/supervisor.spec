...
%define unmangled_version 3.3.1
%define release 1
...
Source1: supervisord.sysconfig
Source2: supervisord.init
Source3: supervisord.conf
License: BSD-derived (http://www.repoze.org/LICENSE.txt)
Group: Development/Libraries
BuildRoot: %{_tmppath}/%{name}-%{version}-%{release}-buildroot
Prefix: %{_prefix}
BuildArch: noarch
Vendor: Chris McDonough <chrism@plope.com>
Packager: Maarten Diemel <maarten@maartendiemel.nl>
Requires: python
...
install -m 0644 %{_builddir}/%{name}-%{unmangled_version}/supervisord.sysconfig %{buildroot}%{_sysconfdir}/sysconfig/supervisord
install -m 0644 %{_builddir}/%{name}-%{unmangled_version}/supervisord.init %{buildroot}%{_initrddir}/supervisord
install -m 0644 %{_builddir}/%{name}-%{unmangled_version}/supervisord.conf %{buildroot}%{_sysconfdir}/%{name}/supervisord.conf
...
%attr(0755,root,root) %{_sysconfdir}/sysconfig
%attr(0755,root,root) %{_localstatedir}/log/%{name}
%attr(0755,root,root) %config %{_initrddir}/supervisord
%attr(0644,root,root) %config(noreplace) %{_sysconfdir}/%{name}/supervisord.conf
%attr(0644,root,root) %config(noreplace) %{_sysconfdir}/sysconfig/supervisord
